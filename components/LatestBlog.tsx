        ...
        {blogs?.map((blog) => (
          <div key={blog?._id} className="rounded-lg overflow-hidden">
            ...
            <div className="bg-shop_light_bg p-5 ">
              ...
              <Link href={`/blog/${blog.slug?.current}`} className="tex-base font-semibold tracking-wide mt-5 line-clamp-2 hover:text-shop_dark_green hoverEffect">{blog?.title}</Link>
            </div>
          </div>
        ))}
      </div>
